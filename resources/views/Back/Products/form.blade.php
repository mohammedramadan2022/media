...
<div class="row">
    <div class="col-md-6">
        <div class="form-group">
            <x-form.inputs type="number" name="price" slug="form-day-price"></x-form.inputs>
        </div>
        <div class="form-group">
...
<div class="row">
    <div class="col-md-6">
        <div class="form-group">
            <x-form.input-radio :model="$currentModel ?? null" name="has_offer" slug="form-has-offer" color="warning"></x-form.input-radio>
        </div>
    ...
    <div class="col-md-6">
        <div class="form-group">
            <x-form.input-radio :model="$currentModel ?? null" name="has_insurance" slug="form-has-insurance" color="warning"></x-form.input-radio>
        </div>
...
<div class="form-group">
    ...
    <div class="radio radio-dark form-check-inline">
        <input type="radio" id="yes" value="rental" name="ownership" {{ isset($currentModel) ? ($currentModel->ownership ? 'checked' : '') : 'checked' }}>
        <label for="yes">@lang('back.rental')</label>
    </div>

    <div class="radio radio-dark form-check-inline">
        <input type="radio" id="no" value="store" name="ownership" {{ isset($currentModel) ? (!$currentModel->ownership ? 'checked' : '') : '' }}>
        <label for="no">@lang('back.store')</label>
    </div>
</div>

<div class="form-group" id="provider-selector">
    <x-form.select-input :arr="$providers" name="owner_id" slug="providers.t-provider"></x-form.select-input>
</div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            let provider_selector = $('select#owner_id');
            let category_selector = $('select#category_id');
            let providerSelector = $('div#provider-selector');
            ...
            @if(isset($currentModel) && $currentModel->type === "App\Models\Provider")
                provider_selector.val('{{ $currentModel->type_id }}');
                $('div#provider-selector > .col-xs-12 > .form-valid > label').fadeIn();
                provider_selector.next(".select2-container").fadeIn();
            @else
                $('div#provider-selector > .col-xs-12 > .form-valid > label').fadeOut();
                provider_selector.next(".select2-container").hide();
            @endif

            @if(isset($currentModel) && $currentModel->images->count() > 0)
                $.each(@json($currentModel->images->pluck('image', 'id')->toArray()), function(index, name){
            ...
            $('input[name=ownership]').on('change', function () {
                if($(this).val() === 'store') {
                    ...
                    $('div#provider-selector > .col-xs-12 > .form-valid > label').fadeOut();
                    provider_selector.next(".select2-container").fadeOut();
                }
            });
            ...
            function getCategories(section_id) {
                $.ajax({
                    ...
                    method: 'POST',
                    data: {section_id: parseInt(section_id)},
                    success: response => {
