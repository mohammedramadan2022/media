<x-model-trashed-page model="preview">
    <x-slot name="thead">
        <tr>
            <th>#</th>
    ...
    @forelse($trashes as $key => $deleted)
        <tr id="preview-row-{{ $deleted->id }}">
            <td>{{ $key+1 }}</td>
            <td>{{ ucwords($deleted->section->name) }}</td>
            <td><x-table-active-status :status="$deleted->status"></x-table-active-status></td>
            <td>{{ $deleted->created_at->diffForHumans() }}</td>
            <td class="text-center">
                <x-trash-menu table="previews" :model="$deleted"></x-trash-menu>
            </td>
        </tr>
    @empty
