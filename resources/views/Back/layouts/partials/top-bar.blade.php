<div class="navbar-custom">
    <ul class="list-unstyled topnav-menu float-right mb-0">
        <li class="dropdown notification-list">
            <a href="{{ route('admin.logout') }}" onclick="event.preventDefault();document.getElementById('logout-form-0').submit();" class="nav-link dropdown-toggle">
    ...
    <!-- LOGO -->
    <div class="logo-box">
        <a href="{{ route('admin-panel') }}" class="logo text-center">
                ...
                <div class="app-search-box">
                    <div class="input-group">
                        <select class="form-control" name="model_name">
                            ...
                            <option value="admin">@lang('back.admins.admins')</option>
                            <option value="role">@lang('back.roles.roles')</option>
                            <option value="contact">@lang('back.contacts.contacts')</option>
                            @foreach(models() as $key => $model)
                                <option value="{{$key}}">{{ trans('back.'.plural($key).'.'.plural($key)) }}</option>
                            @endforeach
                        </select>
