...
import {addToFave, removeFromFave} from "@/composables/useFave";
import {useRoute} from "vue-router";
import CategoryOptions from "@/components/front/includes/CategoryOptions.vue";

const store = useStore();
const route = useRoute();
const section_id = ref(route.params.id);

store.dispatch('getSectionWithCategories');
store.dispatch('getProductsBySectionId', section_id.value);
...
let has_section_pagination = computed(() => store.state.product.has_section_pagination);
let has_options_pagination = computed(() => store.state.product.has_options_pagination);
            ...
            <div class="container">
                <div class="row gy-3 gy-xl-0">
                    ...
                    <div class="col-xl-3">
                        ...
                        <category-options from="section" :current-route="route"/>
                    </div>
                    ...
                    <div class="col-xl-9">
                                    ...
                                    <div class="col-md-4 col-xl-4">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-select" v-model="filterData.city_id" id="city">
                                                ...
                                                <option v-for="(city, index) in cities" :key="index" :value="city.id">{{city.text}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-4 col-xl-4">
                        ...
                        <div id="results-data" class="row g-3 row-cols-1 row-cols-md-2 row-cols-lg-3 results-data" v-if="allProducts.length > 0">
                            <div class="col" v-for="(product, index) in allProducts" :key="index">
                                <single-product @addToFave="addToFave(product)" @removeFromFave="removeFromFave(product)" :product="product" :key="index" />
                            </div>
                        </div>

                        <div class="row g-3 main-filter-div row-cols-1" style="margin-top: 18px;" v-else>
                            <div class="alert alert-info text-center" v-text="$t('message.noProducts')"></div>
                        </div>

                        <!-- show more -->
                        <div class="text-center" v-if="has_section_pagination">
                            <a href="javascript:void(0);" @click.prevent="store.dispatch('getSectionNextPage', section_id)" class="show-more-link">
                                {{$t('message.loadMore')}}<i class="fa fa-arrow-down ms-2"></i>
                            </a>
                        </div>

                        <div class="text-center" v-if="has_options_pagination">
                            <a href="javascript:void(0);" @click.prevent="store.dispatch('getOptionsNextPage')" class="show-more-link">
                                {{$t('message.loadMore')}}<i class="fa fa-arrow-down ms-2"></i>
                            </a>
                        </div>
                    </div>
